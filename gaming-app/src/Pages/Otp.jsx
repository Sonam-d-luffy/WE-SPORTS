...
const Otp = () => {
  ...
  const handleOTPverification = async(e) => {
    ...
    //console.log(gamerId)
    try {
      const res = await axios.post("http://localhost:5000/api/verify/verify-email" , {gamerId , otp})
      if(res.data.success){
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/verify/resend-otp", { gamerId });
